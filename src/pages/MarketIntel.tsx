// MarketIntel.tsx — Midnight Quant Desk redesign
// Adds: Intelligence department header, Brain Trust agent attribution panel,
// agent status dots on tabs, Midnight Quant panel chrome.
// All existing data logic preserved 100%.

import { useMemo, useState } from "react";
import { Link } from "react-router-dom";
import {
  ArrowRight, Brain, RefreshCw, TrendingUp, TrendingDown,
  Minus, AlertTriangle, ChevronDown, Users2,
} from "lucide-react";
...
function shortName(s: string) { return s.replace("-USD", ""); }
...
function isStaleIntel(iso: string | null | undefined, maxMinutes = 10): boolean {
  if (!iso) return true;
...
function toneClasses(tone: Tone) {
  switch (tone) {
    case "long": case "good": return "bg-status-safe/10 text-status-safe border-status-safe/30";
    case "short": case "warn": return "bg-status-blocked/10 text-status-blocked border-status-blocked/30";
...
function fearGreedTone(score: number | null): Tone { if (score == null) return "neutral"; if (score >= 75 || score <= 25) return "warn"; if (score >= 55) return "long"; if (score <= 45) return "short"; return "neutral"; }
function fundingTone(signal: MarketIntelligence["fundingRateSignal"]): Tone { if (signal === "crowded_long" || signal === "crowded_short") return "warn"; if (signal === "lean_long") return "long"; if (signal === "lean_short") return "short"; return "neutral"; }

function Stat({ label, value }: { label: string; value: string }) {
  return <div><div className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</div><div className="text-sm tabular-nums text-foreground font-mono">{value}</div></div>;
}

function Cond({ label, detail, status }: { label: string; detail: string; status: Dot }) {
  const dot = status === "ok" ? "bg-status-safe" : status === "bad" ? "bg-status-blocked" : "bg-muted-foreground/40";
  return (
...
function IntelBadge({ tone, children }: { tone: Tone; children: React.ReactNode }) {
  return <span className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-md border text-[11px] font-medium", toneClasses(tone))}>{children}</span>;
}
    ...
    <div className="space-y-4">
      ...
      <div className="panel p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Live regime · Taylor</div>
            <div className="text-sm font-medium text-foreground mt-0.5">{shortName(symbol)}</div>
          </div>
          <Link to="/journals" className="text-[11px] text-primary hover:underline inline-flex items-center gap-0.5">journals <ArrowRight className="h-3 w-3" /></Link>
        </div>
...
function BrainTrustPanel({ brief, onRefresh, refreshing }: { brief: MarketIntelligence | undefined; onRefresh: () => void; refreshing: boolean }) {
  const stale = isStaleIntel(brief?.generatedAt, 10);
    ...
    <div className="panel p-4 space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Intelligence · Brain Trust</div>
            <h3 className="text-sm font-semibold text-foreground mt-0.5">Hall · Dollar Bill · Mafee</h3>
            <OwnerBadge owner="Brain Trust" className="mt-1 w-fit" />
            <p className={cn("text-[11px] text-muted-foreground mt-0.5", stale && "text-status-caution")}>
              {stale && <AlertTriangle className="inline h-2.5 w-2.5 mr-0.5 mb-0.5" />}
              {formatRelative(brief.generatedAt)}
            </p>
          </div>
        </div>
        <Button variant="outline" size="sm" onClick={onRefresh} disabled={refreshing} className="h-7 px-2 text-[11px] shrink-0">
      ...
      <div className="flex flex-wrap gap-1.5">
        <IntelBadge tone={bias.tone}><BiasIcon className="h-3 w-3" />{bias.label}<span className="opacity-60">· {Math.round(brief.macroConfidence * 100)}%</span></IntelBadge>
        ...
        <CollapsibleContent className="pt-2 space-y-2">
          ...
          {(brief.nearestSupport != null || brief.nearestResistance != null) && (
            <div className="flex flex-wrap gap-4 text-[11px] pt-1">
              {brief.nearestSupport    != null && <span className="text-muted-foreground">Support <span className="text-foreground font-mono">${brief.nearestSupport.toLocaleString()}</span></span>}
              {brief.nearestResistance != null && <span className="text-muted-foreground">Resistance <span className="text-foreground font-mono">${brief.nearestResistance.toLocaleString()}</span></span>}
            </div>
          )}
          {brief.keyLevelNotes && <p className="text-[11px] text-muted-foreground italic">{brief.keyLevelNotes}</p>}
        </CollapsibleContent>
      </Collapsible>
      ...
      <Collapsible defaultOpen>
        ...
        <CollapsibleContent className="pt-2 space-y-3">
          <p className="text-sm leading-relaxed">{brief.sentimentSummary || <span className="italic text-muted-foreground">Awaiting first run…</span>}</p>
          <div className="flex flex-wrap gap-1.5">
            ...
            <IntelBadge tone={fundingTone(brief.fundingRateSignal)}>
              ...
              {brief.fundingRatePct != null && <span className="opacity-70 font-mono"> · {(brief.fundingRatePct * 100).toFixed(4)}%</span>}
            </IntelBadge>
          </div>
        </CollapsibleContent>
      </Collapsible>
      ...
      <Collapsible defaultOpen>
        ...
        <CollapsibleContent className="pt-2 space-y-2">
          {brief.patternContext ? <p className="text-sm leading-relaxed">{brief.patternContext}</p> : <p className="text-sm italic text-muted-foreground">Awaiting first run…</p>}
          {brief.entryQualityContext && <p className="text-[11px] text-muted-foreground leading-relaxed">{brief.entryQualityContext}</p>}
          {(brief.recentMomentum1h != null || brief.recentMomentum4h != null) && (
            <div className="rounded-md border border-border/70 bg-muted/40 px-2.5 py-1.5 text-[11px] text-muted-foreground flex flex-wrap gap-3">
              {brief.recentMomentum1h != null && <span>1h momentum <span className="text-foreground font-mono">{brief.recentMomentum1h.toFixed(2)}</span></span>}
              {brief.recentMomentum4h != null && <span>4h momentum <span className="text-foreground font-mono">{brief.recentMomentum4h.toFixed(2)}</span></span>}
              {brief.recentMomentumNotes && <span className="italic w-full">{brief.recentMomentumNotes}</span>}
            </div>
          )}
        </CollapsibleContent>
      </Collapsible>
...
export default function MarketIntel() {
  ...
  const researchEntries = useMemo(() => entries.filter((j) => j.kind === "research").slice(0, 6), [entries]);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <SectionHeader
        ...
        title="Market Intel"
        description="Live candle regime alongside AI macro, sentiment, and pattern analysis from Hall, Dollar Bill, and Mafee."
        actions={
      ...
      <div className="panel p-3 flex items-center gap-4 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-muted-foreground">Brain Trust owns this department</span>
        </div>
        <div className="flex items-center gap-3 text-[11px] font-mono text-muted-foreground/70 flex-wrap">
          <span>Hall · macro</span>
          <span className="text-border">·</span>
          <span>Dollar Bill · sentiment</span>
          <span className="text-border">·</span>
          <span>Mafee · patterns</span>
        </div>
      </div>
      ...
      <Tabs value={activeSymbol} onValueChange={(v) => setActiveSymbol(v as SymbolId)}>
        <TabsList className="grid w-full grid-cols-3 h-9 max-w-xs">
          {SYMBOLS.map((s) => {
            ...
            const stale = isStaleIntel(b?.generatedAt, 10);
            return (
              <TabsTrigger key={s} value={s} className="text-[12px] gap-1.5">
                {shortName(s)}
                {b && <span className={cn("rounded border px-1 py-0 text-[10px] leading-4", stale ? "bg-status-blocked/10 text-status-blocked border-status-blocked/30" : "bg-status-safe/10 text-status-safe border-status-safe/30")}>{stale ? "stale" : "live"}</span>}
              </TabsTrigger>
            );
          })}
        </TabsList>
        {SYMBOLS.map((s) => (
          <TabsContent key={s} value={s} className="mt-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <BrainTrustPanel brief={briefsBySymbol.get(s)} onRefresh={handleRefresh} refreshing={refreshing} />
            </div>
          </TabsContent>
        ))}
      </Tabs>
      ...
      {researchEntries.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/journals" className="text-[11px] text-primary hover:underline inline-flex items-center gap-0.5">All journals <ArrowRight className="h-3 w-3" /></Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {researchEntries.map((e) => (
              <Link key={e.id} to="/journals" className="block hover:opacity-90 transition-opacity"><JournalEventCard entry={e} /></Link>
            ))}
          </div>
        </div>
      )}
